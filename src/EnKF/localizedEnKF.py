...
class LocalizedEnsembleKalmanFilter:
    def __init__(
        ...
        localization_radius: Optional[float] = None, # <- added
    ):
        ...
        self.K = np.zeros((xdim, zdim), dtype=np.float32)
        self.S = np.zeros((zdim, zdim), dtype=np.float32)

        self.ensemble_x = self.compute_diag_mvn_sample(x.reshape(-1, 1), diag_cov=self.P, size=N)
        self.ensemble_z = np.zeros((zdim, N), dtype=np.float32)
        ...
        self.localization_matrix = self.build_localization_matrix()
    ...
    def build_localization_matrix(self):
        if self.localization_radius is None:
            return np.ones((self.xdim, self.zdim))
        else:
            dists = np.abs(np.subtract.outer(np.arange(self.xdim), np.arange(self.zdim)))
            loc_matrix = np.exp(-(dists**2) / (2 * self.localization_radius**2))
            return loc_matrix.astype(np.float32)
    ...
    def update(self, z: Optional[np.ndarray]):
        ...
        self.Pzz = 1 / (self.N - 1) * HA @ HA.T
        self.Pzz = self.Pzz * self.localization_matrix.T  # <- localization for Pzz
        self.Pzz += self.R

        self.Pxz = 1 / (self.N - 1) * A @ HA.T
        self.Pxz = self.Pxz * self.localization_matrix  # <- localization for Pxz
...
if __name__ == "__main__":
    ...
    # Kalman Filter
    enkf = LocalizedEnsembleKalmanFilter(x0, fx, N, 2, 1, H, Q, R, P)
    ...
    plt.legend()
    plt.savefig("./results/test/test_localizedEnKF.png")